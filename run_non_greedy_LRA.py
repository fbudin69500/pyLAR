...
def _run_non_greedy_low_rank_atlas(config, software, im_fns, gridSize, verbose=True):
    """unbiased low-rank atlas creation from a selection of images"""
    reference_im_fn = config.reference_im_fn
    result_dir = config.result_dir
    selection = config.selection
...
def setup_and_run(config, software, im_fns, configFN=None, configSoftware=None, fileListFN=None):
    ...
    if REGISTRATION_TYPE == 'BSpline':
        ...
        if not pyLAR.containsRequirements(config, ['gridSize'], configFN):
            return 1
        ...
        if not pyLAR.containsRequirements(config, ['antsParams'], configFN):
            return 1
    ...
    _run_non_greedy_low_rank_atlas(config, software, im_fns, gridSize, True)
